...
Every function declares its OS-authority budget via `@caps(...)`: `@caps()` declares none, and `@caps(fs)` covers `fs::read_file`, `fs::write_file`, etc. `garnet check` follows named calls from each annotated function and reports a declaration that leaves out a capability the chain needs: if `main()` declares `@caps()` and calls a `helper()` that calls `fs::read_file(...)`, the check reports that `main()` does not declare `fs`. Calls through function values or closures are not traced, an unannotated function's own body is not checked, and `garnet run` does not run the checker. At run time, the 20 entry-gated host primitives (file system, outbound network, process, environment, time and UUID v4/v7, log-to-file) trap unless the program's entry function declares the capability. See the [capability enforcement scope table](C_Language_Specification/GARNET_CAPABILITY_ENFORCEMENT_SCOPE.md).
...
The propagator runs at check time. The 20 gated primitives check the caps frames on each call; that cost is unmeasured.
