...
release action.
- 2026-07-31T09:06:00Z: AUTHORSHIP CORRECTION (Part A, reviewer seat) — Lane 2C Verdict 01 commit `29cb1c700c47b48bd4f2902c94c794fd0c6a3cb7` was authored and committed under the fleet-fork identity `Jon Isaac <Navigata1@gmail.com>` while the actual authoring seat was Claude Code on Claude Fable 5 (`claude-fable-5`, Anthropic) on `Pulses-MacBook-Air.local`; the commit's Co-Authored-By trailer disclosed the seat. Per the ceremony authorship ruling, reviewer records from this seat now land under the seat's own identity, beginning with this correction commit. History is not rewritten: `29cb1c7` remains byte-identical historical truth and this entry is the durable correction record. U-30 unchanged: IDC-Trust-Review authors nothing. The implementer's own-identity authorship (`OpenAI Codex <codex@openai.com>`) is consistent with the same ruling.
- 2026-07-31T09:06:30Z: INDEPENDENT REVIEW VERDICT 02 (Claude Code on Claude Fable 5, Anthropic, `Pulses-MacBook-Air.local`; implementer Codex GPT-5-based on the NUC) — **APPROVE at `2576db3`; B1 CLEARED, F2 CURED; product boundary `5cd1136`/`85faad1d` byte-unchanged, Verdict 01 correctness not reopened.** All six memcheck capture hashes recomputed exactly (MANIFEST full-pass); leak tables re-derived from raw text: 0 lost in every category, identical 544 B/1-block pre-main runtime allocation as the only outstanding block in all six; alloc/free balance exactly 1 in all six with alloc counts dropping 271.7x/274.0x/139.7x — the fix removed allocations, not deallocations. Stated limit on record: process-exit memcheck cannot distinguish a collected cycle from drop-freed backing storage — B1 establishes freeing behavior only; cycle-collection correctness rests separately on Verdict 01's decision-equivalence proof and the three named buffered-path tests. Binaries reused not rebuilt (hashes match round-1 provenance at `e73f73d`); captures self-identify base/product candidates; six gates green; successor delta ops/lane2c/** only; whitespace flags are exactly the six valgrind separator-byte files, correctly excluded by the verifier's scoped rule; F3 correctly left out of scope (would have moved the product tree). Part A authorship correction (`2b9a777`) landed before this verdict; both authored under this seat's own identity per the ceremony ruling; U-30 holds. Approval authorizes only Jon's merge decision. STOP.
