...
- The retired vocabulary leaves 84 files under `scripts/` (purge PR-3b):
  ...
  JSON key, dataclass field or test name changes, and the Paper VI anchor
  keeps its pinned wording on both sides of every pair.
  ...
  `docs/truth.json` names an evidence-base commit unreachable from `main`,
  so `foundation_integrity` reads blocked until the base is re-measured.
