...
- **Windows Studio clean-VM proof is read from the repo (path (a)).** The
  ...
  - its manifest matches the files exactly;
  - the record's fields have their JSON types, and `verified` is the literal
    `true`;
...
- Purge PR-3b edits 84 files under `scripts/`: retired words leave their
  comments, docstrings and prose strings, except one printed label in
  `garnet_playground_readiness.py`, and a local variable in five reporters is
  renamed `anchors_ok`. No JSON key, dataclass field or test name changes, so
  39 of those files still carry retired words in identifiers and in the string
  keys and arguments that name them; across `scripts/`, 78 files still do,
  including one comment in `reproduce_ultrapunch.sh`.
- The Paper VI scorecard moves with the reporters in the four documents that
  present it as a current gate's verbatim anchor: `GARNET_v0_5_SLICE_DOGFOOD.md`,
  `GARNET_PROOF_MATRIX.md`, `GARNET_LLM_SUGGEST.md` and
  `GARNET_v0_8_BETA_GATE.md`, alongside the five reporters and the three tests
  that pin it; the Paper VI Exp 3 reporter's explanation of the 10% downgrade
  takes the same wording. Dated decision records, slice ledgers, archived proof
  transcripts, the `.agent` plans and this file's history keep the original
  wording as written.
- `F_Project_Management/LAUNCH/LAUNCH_READINESS.md` is regenerated by its
  reporter rather than hand-edited; that regeneration also records that
  `docs/truth.json` names an evidence-base commit unreachable from `main`, so
  `foundation_integrity` reads blocked until the base is re-measured.
