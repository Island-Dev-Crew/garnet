...
Garnet Studio exposes the same boundary in the macOS workbench: active deterministic converter lanes still use `Convert`, planned-language risk inventory uses `Assist Plan`, and the provider-neutral `Advisory Bundle` action writes a manifested local handoff package under `~/Desktop/dogfood/` without embedding source by default.
