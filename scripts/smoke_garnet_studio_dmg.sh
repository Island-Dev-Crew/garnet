...
for required in "${APP_EXECUTABLE}" "${GARNET_BIN}" "${MATRIX_SCRIPT}" "${OFFLINE_PWA_SMOKE}" "${ADOPTION_STATUS}" "${ASSIST_CONTEXT}" "${ADVISORY_BUNDLE}" "${ADVISORY_HANDOFF}" "${ADVISORY_REVIEW}" "${ASSIST_PLAN}" "${LLM_FEASIBILITY}" "${CONVERTER_STATUS}" "${MAC_CONTINUATION}" "${MIT_DEMO_ROUTE}" "${MIT_STATUS}" "${PROMO_STATUS}" "${PROMO_EXPORT}" "${PROMO_QA}" "${PROMO_RENDER}" "${PROMO_SYNC}" "${READINESS_STATUS}" "${NOTARIZATION_STATUS}"; do
  if [ ! -x "${required}" ]; then
    echo "error: copied app is missing executable asset: ${required}" >&2
    exit 4
  fi
done
record "pass" "Copied app executable assets exist" "app executable, bundled CLI, matrix script, status reporters, MIT demo-route reporter, Mac-side continuation reporter, converter advisory bundle, converter advisory handoff packet, converter advisory review gate, converter LLM feasibility reporter, promo render, visual-QA, website-export, site-sync harnesses, PWA smoke" "None."
