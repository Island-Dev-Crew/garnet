#!/usr/bin/env python3
"""Record Studio domain-matrix shell proof on Windows and WSL.

This is a Windows-lane productization proof. It exercises the Tauri Studio
binary's `--studio-domain-proof-smoke` command, which routes through the same
Rust command wrapper as the UI's Domain Proof Matrix button. The WSL row is
...
SUMMARY_NAME = "garnet-studio-domain-shell-proof.json"
MARKDOWN_NAME = "garnet-studio-domain-shell-proof.md"
MANIFEST_NAME = "MANIFEST.sha256"
SCHEMA = "garnet.studio.domain_shell_proof.v1"
WINDOWS_ROOT = ROOT / "proofs" / "windows" / "studio-domain-shell"
WSL_ROOT = ROOT / "proofs" / "linux" / "execution" / "studio-domain-shell"
...
def _copied_payload_summary(bundle_dir: Path, payload_dir: str) -> dict[str, object]:
    ...
    payload_json = payload / "domain-proof-shell-smoke.json"
    try:
    ...
    return {
        "copied": True,
        "payload_dir": payload_dir,
        "payload_json": f"{payload_dir}/domain-proof-shell-smoke.json",
...
def record_windows(*, output_dir: Path, build: bool = True) -> Path:
    ...
    env["GARNET_STUDIO_DOMAIN_MATRIX_ROOT"] = str(
        ROOT / "target" / f"domain-shell-matrix-{timestamp_slug()}"
    )
    ...
    commands.append(
        _run_command(
            command_id="studio-domain-proof-smoke",
            command=[str(binary), "--studio-domain-proof-smoke"],
            display_args=[_repo_relative(binary), "--studio-domain-proof-smoke"],
            bundle_dir=output_dir,
            env=env,
        )
    )
    stdout = (output_dir / "commands" / "studio-domain-proof-smoke-stdout.txt").read_text(
        encoding="utf-8"
    )
    ...
    passed = all(command.status == "passed" for command in commands) and payload["payload_status"] == "passed"
    data: dict[str, object] = {
        "schema": SCHEMA,
        "status": "passed" if passed else "failed",
        "created_at": datetime.now(UTC).isoformat(),
        "target_platform": "windows",
        "platform_tier": "windows-local-tauri-domain-shell-proof",
        "source_included": False,
        "provider_api_called": False,
        "domain_matrix_shell_proven": payload["payload_status"] == "passed",
...
def record_wsl(*, output_dir: Path, build: bool = True) -> Path:
    ...
    home = f"{wsl_root}/target/wsl-domain-shell-home-{stamp}"
    matrix_root = f"{wsl_root}/target/wsl-domain-shell-matrix-{stamp}"
    commands.append(
        _run_command(
    ...
    smoke_script = (
        ...
        f"{sh_quote(matrix_root)} && "
        f"HOME={sh_quote(home)} GARNET_REPO={sh_quote(wsl_root)} "
        f"GARNET_STUDIO_DOMAIN_MATRIX_ROOT={sh_quote(matrix_root)} "
        "./target/release/garnet-studio --studio-domain-proof-smoke"
    )
    commands.append(
        _run_command(
            command_id="wsl-studio-domain-proof-smoke",
            ...
            display_args=[
                ...
                "bash",
                "-lc",
                "cd <repo> && HOME=<repo>/target/wsl-domain-shell-home-* ./target/release/garnet-studio --studio-domain-proof-smoke",
            ],
            bundle_dir=output_dir,
        )
    )
    stdout = (output_dir / "commands" / "wsl-studio-domain-proof-smoke-stdout.txt").read_text(
        encoding="utf-8"
    )
...
def verify_summary(path: Path, *, expected_platform: str) -> bool:
    ...
    if manifest is None:
        return False
    if not {SUMMARY_NAME, MARKDOWN_NAME, "studio-payload/domain-proof-shell-smoke.json"}.issubset(
        manifest
    ):
        return False
    ...
    for command in commands:
        ...
        if stdout_file not in manifest or stderr_file not in manifest:
            return False
    try:
        payload = json.loads((bundle_dir / "studio-payload" / "domain-proof-shell-smoke.json").read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return False
    common_ok = (
        ...
        and data.get("domain_matrix_shell_proven") is True
        ...
        and payload.get("mode") == "studio-domain-proof-smoke"
        and payload.get("domain_matrix_command_success") is True
        and payload.get("stdout_has_domain_matrix") is True
    )
    if not common_ok:
        return False
    ...
    if expected_platform == "windows":
        return (
            data.get("platform_tier") == "windows-local-tauri-domain-shell-proof"
...
def read_committed_evidence(repo_root: Path = ROOT) -> DomainShellEvidence:
    windows = _verified_summary_under(
        repo_root / "proofs" / "windows" / "studio-domain-shell",
        expected_platform="windows",
    )
    wsl = _verified_summary_under(
        repo_root / "proofs" / "linux" / "execution" / "studio-domain-shell",
        expected_platform="wsl",
    )
    if windows is None or wsl is None:
        return DomainShellEvidence(
            False,
            windows,
            wsl,
            (
                "No complete committed Windows + WSL Studio domain-shell proof pair exists yet. "
                "Expected Windows Tauri `--studio-domain-proof-smoke` and WSL portability bundles."
            ),
        )
    return DomainShellEvidence(
        True,
        ...
        wsl,
        (
            f"Committed Windows Studio domain-shell proof: `{_repo_display(windows)}`. "
            f"Committed WSL Studio domain-shell portability proof: `{_repo_display(wsl)}`. "
...
def render_markdown(data: dict[str, object]) -> str:
    lines = [
        ...
        f"- Domain matrix shell proven: `{str(data.get('domain_matrix_shell_proven')).lower()}`",
