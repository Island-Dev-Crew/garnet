...
class _ObjectReader:
    ...
    def _drain_stderr(self) -> None:
        ...
        try:
            ...
            # A lost error stream could hide graft advice: poison the reader
            # and let ``close`` report it, so the run cannot finish green.
            self._stderr = b""
    ...
    def close(self) -> list[str]:
        ...
        for stream in (process.stdout, process.stderr):
            try:
                ...
                pass
        problems: list[str] = []
        if self._stderr_failed:
            problems.append("git object reader error stream could not be read")
        if b"graft" in self._stderr.lower():
            problems.append("git object reader reported graft advice")
        return problems
...
def verify_landed_review_marker(
    ...
    *,
    ...
    """Verify one landed marker, then close the batched object readers.

    The readers' close findings (graft advice, a lost error stream) join the
    marker findings, and no ``cat-file`` child outlives the call.  Callers
    inside a larger verification use ``_landed_review_marker_findings`` so the
    shared reader stays open until that verification closes it.
    """
    findings: list[str] = []
    try:
        findings.extend(
            _landed_review_marker_findings(
                marker, root=root, main_ref=main_ref, marker_path=marker_path
            )
        )
    finally:
...
def _landed_review_marker_findings(
    marker: object,
    *,
    root: Path = ROOT,
    main_ref: str = "refs/remotes/origin/main",
    ...
    for marker_path in valid_markers:
        ...
        if marker is not None:
            ...
            findings.extend(
                ...
                for problem in _landed_review_marker_findings(
                    marker,
                    root=root,
                    main_ref=main_ref,
...
def main(argv: list[str] | None = None, *, root: Path = ROOT) -> int:
    ...
    if args.print_trust_surface:
        ...
        live.extend(_close_object_readers())
        print(json.dumps(
