#!/usr/bin/env python3
"""`@caps` host-authority runtime enforcement status (S90).
...
Host-authority surfaces only (env / process / fs / log-to-file); pure computation
is unaffected, and outside any managed-program frame (direct host/test calls) there
is no `@caps` context to enforce, so such calls are allowed. The **VM** backend does
not yet enforce `@caps`. Mac-authored + Mac-tested; the Windows trap re-proves via
the cross-OS `cargo test` matrix (recorded Windows-proof-pending).
"""
...
def read_status() -> CapsEnforcementStatus:
    ...
    ok = has_require and not missing and tests_present
...
def render_markdown(r: CapsEnforcementStatus) -> str:
    return "\n".join([
        "# Garnet @caps host-authority runtime enforcement status (S90)",
        "",
        ...
        f"- env/proc/fs/pure enforcement tests present: "
        f"{'yes' if r.enforcement_tests_present else 'NO'}",
        "",
        "Host-authority surfaces only (env/process/fs/log-to-file); pure computation "
        "unaffected; calls outside a managed-program frame are allowed. The VM "
        "backend does not yet enforce @caps. Windows trap re-proves via the cross-OS "
        "matrix (Windows-proof-pending).",
        "",
    ])
