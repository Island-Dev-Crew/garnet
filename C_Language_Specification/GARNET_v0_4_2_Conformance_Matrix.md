...
| §4.5 ARC + Bacon–Rajan cycle detection | 🟡 | `garnet-memory-v0.3/src/{alloc,cycle}.rs`; `garnet-memory-v0.3/tests/cycle.rs`; `deferred_arc_cycle_detection`; cycle-aware store-root property tests | Phase 6Q adds concrete `CycleAwareKindAllocator` root-release evidence for buffered trial candidates, deterministic finalization order, collected nodes, and safe-affine exclusion on top of the bounded trial-deletion cycle model and Phase 6K store-root lifecycles. Production allocator-integrated ARC remains Roadmap T3.1 work. |
