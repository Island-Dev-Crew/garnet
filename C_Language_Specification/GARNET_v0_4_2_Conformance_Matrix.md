...
| §8.6 Borrow-check rules B1–B5 | 🟡 | `garnet-check-v0.3/src/borrow.rs`; `partial_borrow_rule_suite`; `deferred_full_borrow_rule_suite`; `garnet-check-v0.3/tests/borrow.rs` | Phase 4I activates conformance for direct safe-mode B1/B2 mut-alias rejection, method receiver aliasing, simple field-place aliasing and field use-after-move, conservative index-place aliasing and index use-after-move with nested index operand checks, B4 use-after-move through `own` parameters, unambiguous `own self` method receiver moves, simple typed receiver disambiguation for same-named impl methods, a B5 drop-discipline slice that rejects overlapping places passed as `own` in the same call, direct-returning `if`/`else` branch liveness, direct `return` block termination, and direct-returning `while`/`loop` body liveness. Dynamic places, full B3 lifetime containment beyond elision, broader CFG/drop elaboration, general loop fixed-point analysis, for-loop liveness, generic/trait impl dispatch, and two-phase borrows remain partial/deferred. |
...
| §7 Errors | 3 | — | — | — | Complete. |
| §8 Safe-mode | 1 | 1 | 2 | — | Phase 4I activates partial borrow-rule conformance, a conservative lifetime-elision subset for reference returns, B5 same-call drop discipline, and first direct-return CFG-liveness slices; full CFG NLL/lifetime inference and formal theorem stay paper-side. |
...
| **Totals** | **32** | **3** | **8** | **11** | 54 tracked items after v0.5 Phase 5C trait/generic slices. |
