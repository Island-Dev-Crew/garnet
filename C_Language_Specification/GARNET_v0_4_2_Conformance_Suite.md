...
| §8.6 full place-granular borrow rules B1-B5 | v0.5 Phase 4K | Safe-Mode Gate | `deferred_full_borrow_rule_suite` | active partial pass for B5 same-call overlapping `own` drop-discipline rejection plus direct-returning branch/block/loop-body liveness, scoped `for` loop variables, and scoped `match` pattern bindings; full CFG/drop elaboration and match-arm block statement preservation remain deferred |
