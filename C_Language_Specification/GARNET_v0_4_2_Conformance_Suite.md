...
| §6.3 full match exhaustiveness | v0.5 Phase 4V | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability` | active partial pass for `Bool`, same-module enum subjects, finite nested-constructor payloads, scoped named/glob/module-qualified imported enum aliases, immutable local boolean/enum initializer inference, direct mutable-local finite assignment inference, conservative `if`/`elsif`/`else` assignment joins including nested all-path `if` branch assignments, non-finite and compound mutable assignment invalidation, literal guard reasoning, and open-domain literal/catch-all reachability; cross-file/package imports, recursive/open payload reasoning, loop/try/closure-merged assignment flow, broader expression/type inference, open-domain exhaustiveness/range reasoning, and non-literal guard reasoning remain deferred |
