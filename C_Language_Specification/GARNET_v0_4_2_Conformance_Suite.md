...
| §8.6 partial borrow rules | v0.5 Phase 4J | Safe-Mode Gate | `partial_borrow_rule_suite`; `deferred_full_borrow_rule_suite` | active pass for B1/B2 direct mut-alias rejection, method receiver aliasing, simple field-place aliasing/field moves, conservative index-place aliasing/index moves with nested index operand checks, B4 direct function use-after-move, unambiguous `own self` method receiver use-after-move, simple typed receiver disambiguation for same-named impl methods, B5 same-call overlapping `own` drop-discipline rejection, direct-returning `if`/`else` branch liveness, direct `return` block termination, direct-returning `while`/`loop`/`for` body liveness, and loop-variable shadowing that preserves outer moved state |
...
| §8.6 full place-granular borrow rules B1-B5 | v0.5 Phase 4J | Safe-Mode Gate | `deferred_full_borrow_rule_suite` | active partial pass for B5 same-call overlapping `own` drop-discipline rejection plus direct-returning branch/block/loop-body liveness and scoped `for` loop variables; full CFG/drop elaboration remains deferred |
