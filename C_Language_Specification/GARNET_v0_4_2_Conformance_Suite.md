...
| §6.3 safe-mode match coverage | v0.5 Phase 4O | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability`; `garnet-check-v0.3/tests/match_coverage.rs` | active pass for finite-domain `Bool`, same-module enum subject, finite nested-constructor payload, and scoped named/glob/module-qualified imported enum alias exhaustiveness, guarded arms not counting as exhaustive coverage, duplicate covered-arm diagnostics, and arms after unguarded catch-all diagnostics |
...
| §6.3 full match exhaustiveness | v0.5 Phase 4O | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability` | active partial pass for `Bool`, same-module enum subjects, finite nested-constructor payloads, and scoped named/glob/module-qualified imported enum aliases; cross-file/package imports, recursive/open payload reasoning, open-domain literal reasoning, and general guard reasoning remain deferred |
