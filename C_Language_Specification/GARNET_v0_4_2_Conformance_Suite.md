...
| §6.3 safe-mode match coverage | v0.5 Phase 4M | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability`; `garnet-check-v0.3/tests/match_coverage.rs` | active pass for finite-domain `Bool` and same-module enum subject exhaustiveness, guarded arms not counting as exhaustive coverage, duplicate covered-arm diagnostics, and arms after unguarded catch-all diagnostics |
...
| §6.3 full match exhaustiveness | v0.5 Phase 4M | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability` | active partial pass for `Bool` and same-module enum subjects; nested constructor exhaustiveness, imported enum resolution, open-domain literal reasoning, and general guard reasoning remain deferred |
