...
| §4.5 trial-deletion cycle fixtures | v0.5 Phase 6C | Memory Core Gate | `deferred_arc_cycle_detection`; `garnet-memory-v0.3/tests/cycle.rs` | active pass for rooted retention, trial candidates, scan-black retained candidates, deterministic finalization-order reporting, safe-mode affine allocation exclusion, unrooted cycle collection, unrooted acyclic retention, and kind-scheduled cross-kind collection |
...
| §8.6 partial borrow rules | v0.5 Phase 4E | Safe-Mode Gate | `partial_borrow_rule_suite` | active pass for B1/B2 direct mut-alias rejection, method receiver aliasing, simple field-place aliasing/field moves, conservative index-place aliasing/index moves with nested index operand checks, B4 direct function use-after-move, unambiguous `own self` method receiver use-after-move, and simple typed receiver disambiguation for same-named impl methods |
...
|---|---|---|---|
| §4.5 production ARC + Bacon-Rajan allocator integration | v0.5 Phase 6 | Memory Core Gate | active Phase 6A/6B/6C fixtures plus future allocator-integrated collector tests |
