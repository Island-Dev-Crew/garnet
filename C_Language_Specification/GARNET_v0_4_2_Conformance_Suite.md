...
| §6.3 safe-mode match coverage | v0.5 Phase 4Q | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability`; `garnet-check-v0.3/tests/match_coverage.rs` | active pass for finite-domain `Bool`, same-module enum subject, finite nested-constructor payload, scoped named/glob/module-qualified imported enum alias exhaustiveness, literal `if true` guard coverage, literal `if false` guard reachability, duplicate covered-arm diagnostics, open-domain duplicate literal diagnostics, and arms after unguarded catch-all diagnostics |
...
| §6.3 full match exhaustiveness | v0.5 Phase 4Q | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability` | active partial pass for `Bool`, same-module enum subjects, finite nested-constructor payloads, scoped named/glob/module-qualified imported enum aliases, literal guard reasoning, and open-domain literal/catch-all reachability; cross-file/package imports, recursive/open payload reasoning, open-domain exhaustiveness/range reasoning, and non-literal guard reasoning remain deferred |
