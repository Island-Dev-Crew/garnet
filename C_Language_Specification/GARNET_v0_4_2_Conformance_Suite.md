...
| §6.3 safe-mode match coverage | v0.5 Phase 4BI | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability`; `garnet-check-v0.3/tests/match_coverage.rs` | active pass for finite-domain `Bool`, same-module enum subject, finite nested-constructor payload, scoped named/glob/module-qualified imported enum alias exhaustiveness, immutable local boolean/enum initializer inference, direct mutable-local finite assignment inference, conservative `if`/`elsif`/`else` assignment joins including nested all-path `if` branch assignments, non-finite and compound mutable assignment invalidation, loop-body assignment invalidation with ordered shadowing checks, `try`/`rescue`/`ensure` assignment invalidation, uninvoked closure definition boundaries, immediate closure-literal invocation invalidation, direct local closure-literal binding call invalidation, branch-joined local closure-literal binding call invalidation, branch-rebound local closure-literal binding call invalidation, local closure-alias binding call invalidation, branch-joined local closure-alias binding call invalidation, direct branch-selected closure expression call invalidation, literal `if true` guard coverage, literal `if false` guard reachability, immutable local, same-module top-level, scoped named/glob imported top-level, and path-qualified top-level boolean guard-constant, alias, and boolean-const-expression coverage/non-coverage, including short-circuit, boolean equality/inequality, direct boolean guard-expression, checked integer arithmetic/equality/inequality/relational cases plus same-module bare-name and scoped named/glob imported integer const identifier cases with parameter shadowing, and static boolean relational const guard cases, static nil/symbol/string equality/inequality const guard cases including static interpolated strings, static string relational const guard cases, static nil relational const guard cases, mixed known-literal equality/inequality facts, plus finite float equality/inequality, int-float equality, finite float/int-float relational facts, and finite float/int-float arithmetic facts, and immutable local boolean/integer const-expression guard aliases that can reference path-qualified top-level constants, duplicate covered-arm diagnostics, open-domain duplicate literal diagnostics, and arms after unguarded catch-all diagnostics |
...
|---|---|---|---|
| §4.5 production ARC + Bacon-Rajan allocator integration | v0.5 Phase 6 | Memory Core Gate | active Phase 6A-6R fixtures, including concrete `CycleAwareKindAllocator` root-release finalization and safe-affine exclusion evidence (Phase 6Q) plus allocator-facing buffered edge-removal collection evidence at threshold-crossing and below-threshold decrements across all four memory kinds (Phase 6R); production allocator-integrated ARC and runtime finalizer invocation remain deferred |
| §6.3 full match exhaustiveness | v0.5 Phase 4BI | Safe-Mode Gate | `deferred_match_exhaustiveness_and_reachability` | active partial pass for `Bool`, same-module enum subjects, finite nested-constructor payloads, scoped named/glob/module-qualified imported enum aliases, immutable local boolean/enum initializer inference, direct mutable-local finite assignment inference, conservative `if`/`elsif`/`else` assignment joins including nested all-path `if` branch assignments, non-finite and compound mutable assignment invalidation, loop-body assignment invalidation with ordered shadowing checks, `try`/`rescue`/`ensure` assignment invalidation, uninvoked closure definition boundaries, immediate closure-literal invocation invalidation, direct local closure-literal binding call invalidation, branch-joined local closure-literal binding call invalidation, branch-rebound local closure-literal binding call invalidation, local closure-alias binding call invalidation, branch-joined local closure-alias binding call invalidation, direct branch-selected closure expression call invalidation, literal guard reasoning, immutable local, same-module top-level, scoped named/glob imported top-level, and path-qualified top-level boolean guard-constant, alias, and boolean-const-expression reasoning, including short-circuit, boolean equality/inequality, direct boolean guard-expression, checked integer arithmetic/equality/inequality/relational cases plus same-module bare-name and scoped named/glob imported integer const identifier cases with parameter shadowing, static boolean relational const guard cases, static nil/symbol/string equality/inequality const guard cases including static interpolated strings, static string relational const guard cases, static nil relational const guard cases, mixed known-literal equality/inequality facts, plus finite float equality/inequality, int-float equality, finite float/int-float relational facts, and finite float/int-float arithmetic facts, and immutable local boolean/integer const-expression guard aliases that can reference path-qualified top-level constants, and open-domain literal/catch-all reachability; cross-file/package imports, recursive/open payload reasoning, non-finite floats, call-backed/dynamic interpolated strings, broader non-boolean non-string non-numeric comparison, broader float edge-case reasoning, function-call, and broader const expression evaluation beyond immutable local aliases and path-qualified const references, loop fixed-point domain inference, broader mutable/escaped/general higher-order closure invocation/call-effect analysis, broader expression/type inference, open-domain exhaustiveness/range reasoning, and broader non-literal guard reasoning remain deferred |
