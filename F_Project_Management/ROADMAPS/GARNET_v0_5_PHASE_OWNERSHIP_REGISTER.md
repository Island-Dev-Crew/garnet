...
| 4 Safe-mode ownership | `garnet-check-v0.3/src/borrow.rs`, `garnet-cli/tests/conformance_skeleton.rs` | Phase 4D active | full NLL/index-place B1-B5 plus generic/trait impl dispatch still pending | `cargo test -p garnet-cli --test conformance_skeleton partial_borrow_rule_suite`; `cargo test -p garnet-check --test borrow` | full borrow/NLL checker handoff |
