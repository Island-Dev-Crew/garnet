...
| 3 Actors + Sendable | `garnet-actor-runtime`, checker, interpreter bridge | not started | actor crate remains detached from Garnet source | `cargo test -p garnet-cli new_cmd` plus agent template smoke | agent-orchestrator actor-mode template |
| 4 Safe-mode ownership | `garnet-check-v0.3/src/borrow.rs` | not started | under-checking creates false safety claims | `cargo test -p garnet-cli --test conformance_skeleton partial_borrow_rule_suite -- --ignored` | safe-mode checker handoff |
