...
| 6 Memory Core | `garnet-memory-v0.3` | Phase 6A active | production ARC/cycle work is algorithmically subtle; current evidence is a bounded reference model | `cargo test -p garnet-memory --test cycle`; `cargo test -p garnet-cli --test conformance_skeleton deferred_arc_cycle_detection` | Memory Core implementation addendum |
