...
| 5 Traits/generics | `garnet-check-v0.3`, future lowering | Phase 5A active | zero-cost claims outrun backend evidence; generic overlap solving still pending | `cargo test -p garnet-check --test coherence`; `cargo test -p garnet-cli --test conformance_skeleton deferred_trait_coherence`; `cargo test -p garnet-cli --test conformance_skeleton parsed_only_monomorphization -- --ignored` | trait/generic implementation memo |
