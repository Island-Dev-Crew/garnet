...
| Compiler-as-agent cache privacy | Partial Phase 6F | `garnet-cli/src/cmd/mod.rs`; `garnet-cli/tests/cache_episodes.rs`; cache episode logs redact external absolute paths and collapse project-local absolute paths to stable relative labels; CacheHMAC and ProvenanceStrategy tests remain active | Add broader multi-process machine-key stress and committed-cache abuse-case smokes |
