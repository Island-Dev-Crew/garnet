...
| Garnet Studio agentic matrix surface | Active for source-checkout, packaged app, mounted-DMG install-smoke, and notarization-preflight workflows | `swift run --package-path apps/garnet-studio-macos GarnetStudio --agentic-matrix-test`; `./scripts/package_garnet_studio_macos.sh`; `scripts/smoke_garnet_studio_dmg.sh target/macos/GarnetStudio.dmg`; `scripts/preflight_garnet_studio_notarization.sh --copy-to-desktop`; Desktop bundle `/Users/idc2.0/Desktop/dogfood/garnet-studio-notarization-preflight-20260515-214525` | Resolve Developer ID identity/profile/hardened-runtime blockers, then web/PWA and mobile gates as separate productization slices |
