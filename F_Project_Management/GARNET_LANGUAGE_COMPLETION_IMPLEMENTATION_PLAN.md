...
| Promo video readiness contract | Active as composition-ready evidence for the future 30-second Garnet promo/ad lane, with local rendered-artifact evidence when Desktop MP4/WebM outputs are present | `scripts/garnet_promo_video_status.py`; `scripts/render_garnet_promo_video.mjs`; `docs/promo/DESIGN.md`; `docs/promo/composition.html`; `python3 scripts/test_garnet_promo_video_status.py`; `python3 scripts/test_render_garnet_promo_video.py`; `scripts/run_agentic_dogfood_matrix.py` probes `report-promo-video-*` | Keep visual QA, website export, Desktop acceptance, and overclaim checks as required gates before claiming a shipped video |
