...
| Agentic dogfood stress matrix | Active for audited agent-facing workflows, packaged-app mode, and macOS CI artifact capture | `scripts/run_agentic_dogfood_matrix.py --copy-to-desktop --strict`; `.github/workflows/agentic-dogfood-matrix.yml` runs the headless-safe 22-probe matrix with `--skip-app-workbench`; Desktop bundle `/Users/idc2.0/Desktop/dogfood/garnet-agentic-dogfood-20260515-220620`; `F_Project_Management/DOGFOOD/GARNET_AGENTIC_DOGFOOD_STRESS_PLAN.md` | Keep local/Desktop and CI matrix evidence aligned; broaden only when new productization lanes have their own probes |
...
| Garnet web/PWA readiness | Active seed installable docs surface plus live Pages smoke | `docs/manifest.webmanifest`; `docs/service-worker.js`; `scripts/smoke_garnet_web_pwa.sh --copy-to-desktop --strict`; `scripts/smoke_garnet_pages_pwa.sh --copy-to-desktop --strict`; `.github/workflows/web-pwa-readiness.yml` | Broaden beyond static docs into a richer web workbench only after the shell, offline cache, and Pages deployment smoke stay green |
