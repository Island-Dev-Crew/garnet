...
| Promo video readiness contract | Active as source-locked evidence for the future 30-second Garnet promo/ad lane | `scripts/garnet_promo_video_status.py`; `python3 scripts/test_garnet_promo_video_status.py`; `scripts/run_agentic_dogfood_matrix.py` probes `report-promo-video-*` | Keep HyperFrames/Remotion composition, rendered MP4/WebM, visual QA, website export, and Desktop dogfood evidence as required gates before claiming a shipped video |
