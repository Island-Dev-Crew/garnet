...
| Garnet Studio agentic matrix surface | Active for source-checkout, packaged app, and mounted-DMG install-smoke workflows | `swift run --package-path apps/garnet-studio-macos GarnetStudio --agentic-matrix-test`; `./scripts/package_garnet_studio_macos.sh`; `scripts/smoke_garnet_studio_dmg.sh target/macos/GarnetStudio.dmg`; Desktop bundle `/Users/idc2.0/Desktop/dogfood/garnet-agentic-dogfood-20260515-212912` | Add signed/notarized distribution, then web/PWA and mobile gates as separate productization slices |
