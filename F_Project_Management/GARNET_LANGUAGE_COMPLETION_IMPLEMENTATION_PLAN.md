...
| Compiler-as-agent cache privacy/replay | Partial Phase 6G | `garnet-cli/src/{cache,cmd}.rs`; `garnet-cli/tests/cache_episodes.rs`; cache episode logs redact external absolute paths, collapse project-local absolute paths to stable relative labels, and warn while ignoring same-cache foreign-key plus copied-cache replay episodes; CacheHMAC and ProvenanceStrategy tests remain active | Add broader strategy replay and long-running concurrent write stress |
