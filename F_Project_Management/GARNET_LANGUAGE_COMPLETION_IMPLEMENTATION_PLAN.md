...
| MIT deck-preview artifact | Active as a browser-smoked HTML review artifact, not final MIT/productization acceptance | `scripts/garnet_mit_deck_preview.py`; `scripts/smoke_garnet_mit_deck_preview_browser.mjs`; `python3 scripts/test_garnet_mit_deck_preview.py`; `python3 scripts/test_garnet_mit_deck_preview_browser_smoke.py`; `scripts/run_agentic_dogfood_matrix.py` probes `report-mit-deck-preview-*` | Render the current deck outline into a self-contained HTML preview plus JSON, outline Markdown, and checksum manifest, then verify it through local headless Chrome for desktop/mobile layout, screenshot evidence, no external assets, and readiness-boundary copy without treating it as human/aesthetic deck approval |
...
Phase 6BK MIT deck-preview browser-smoke pass:
`scripts/smoke_garnet_mit_deck_preview_browser.mjs` now turns the
browser-smokeable deck-preview claim into Chrome/CDP evidence. It generates and
manifest-verifies the deck preview, serves it locally, checks desktop and mobile
viewports for no horizontal overflow, verifies that the artifact is
self-contained and keeps the `58.6%`, `87/87`, speaker-note, evidence, and
forbidden-claim boundaries visible, captures a PNG screenshot, and writes a
manifested browser-smoke report. The first live browser run found horizontal
overflow from long evidence paths, so the deck CSS now wraps long text and
inline code. Source-checkout evidence passes `135/135` with `skipped=0` in
`/Users/idc2.0/Desktop/dogfood/garnet-agentic-dogfood-20260516-200458`; browser
smoke evidence passes in
...
Phase 6BM converter provider-option registry pass:
`scripts/garnet_converter_llm_feasibility.py` now turns the provider discussion
into a machine-readable registry rather than loose roadmap copy. The registry
lists ten candidates (OpenAI GPT-5.5 class, Claude Opus/Sonnet class, Grok code,
Kimi/Moonshot K-series, Gemini/Gemma, DeepSeek coder, Qwen coder, local 1.58-bit,
a domain-fine-tuned Garnet adapter, and a multi-model reviewer quorum) while
keeping every option advisory-only, disabled by default, source-omitted by
default, privacy-review-gated, and human-approval-gated. The adoption and
MIT-readiness reporters, public landing/status pages, and converter strategy
doc now point to that registry as an evaluation contract. The agentic matrix
adds `report-converter-llm-feasibility-provider-options` and current
source-checkout evidence passes `137/137` with `skipped=0` in
`/Users/idc2.0/Desktop/dogfood/garnet-agentic-dogfood-20260516-210126`. This
