...
- Memory Core ARC/cycle work now has Phase 6A executable reference evidence:
  `CycleGraph` fixtures retain rooted nodes, collect unrooted cycles, leave
  unrooted acyclic nodes for ordinary retention/eviction, and scan cross-kind
  cycles by kind partition. This is not the production allocator-integrated
  Bacon-Rajan collector, finalizer ordering, or safe-mode cycle interaction.
