...
| 4 Safe mode | unsafe ownership examples are rejected, valid examples pass | Phase 4B active: top-level conformance now rejects direct use-after-move through `own` parameters, direct `mut`+`borrow` aliasing, unambiguous `own self` method receiver use-after-move, and method receiver aliasing while preserving managed ARC semantics. Full NLL/lifetime inference, place-granular B1-B5, type-resolved impl dispatch, and drop discipline remain pending. | `cargo test -p garnet-cli --test conformance_skeleton partial_borrow_rule_suite`; `cargo test -p garnet-check --test borrow` |
