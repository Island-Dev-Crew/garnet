...
| 2 Managed runtime | Parser-stage features run in managed mode | Phase 2C in progress: syntactic `do...end` block invocation, `yield`, `next`, per-instance `@dynamic` method tables, and protocol-typed managed parameter checks are active; static impl fallback, method_missing, runtime `as Protocol` casts, and full signature compatibility remain deferred | `cargo test -p garnet-cli --test conformance_skeleton`; `cargo test -p garnet-cli --test conformance_phase_gates` |
