...
| 4 Safe mode | unsafe ownership examples are rejected, valid examples pass | Phase 4A active: top-level conformance now rejects direct use-after-move through `own` parameters and direct `mut`+`borrow` aliasing while preserving managed ARC semantics. Full NLL/lifetime inference, place-granular B1-B5, method-call ownership, and drop discipline remain pending. | `cargo test -p garnet-cli --test conformance_skeleton partial_borrow_rule_suite`; `cargo test -p garnet-check --test borrow` |
