...
| 2 Managed runtime | Parser-stage features run in managed mode | Phase 2E in progress: syntactic `do...end` block invocation, `yield`, `next`, per-instance `@dynamic` method tables, static inherent impl fallback, `method_missing`, protocol-typed managed parameter checks, and protocol method signature checks are active; runtime `as Protocol` casts, generic method unification, built-in typed signatures, and `@dynamic impl` tables remain deferred | `cargo test -p garnet-cli --test conformance_skeleton`; `cargo test -p garnet-cli --test conformance_phase_gates` |
