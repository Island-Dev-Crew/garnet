...
| 5 Traits/generics | trait/generic examples check and run honestly | Phase 5A active: exact duplicate trait impls and orphan impls are rejected while local-trait and local-type impls remain accepted. Generic overlap solving, imported-package coherence, interpreter-level generic instantiation, and native monomorphization remain pending. | `cargo test -p garnet-check --test coherence`; `cargo test -p garnet-cli --test conformance_skeleton deferred_trait_coherence` |
