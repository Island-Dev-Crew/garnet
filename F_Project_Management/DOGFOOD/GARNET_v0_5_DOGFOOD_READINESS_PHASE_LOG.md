...
| 2 Managed runtime | Parser-stage features run in managed mode | Phase 2D in progress: syntactic `do...end` block invocation, `yield`, `next`, per-instance `@dynamic` method tables, static inherent impl fallback, `method_missing`, and protocol-typed managed parameter checks are active; runtime `as Protocol` casts, full signature compatibility, and `@dynamic impl` tables remain deferred | `cargo test -p garnet-cli --test conformance_skeleton`; `cargo test -p garnet-cli --test conformance_phase_gates` |
