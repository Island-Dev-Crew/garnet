...
| 3 Actors + Sendable | `agent-orchestrator` uses actor syntax frictionlessly and nonsendable actor payloads are rejected | Phase 3A active for `@nonsendable` actor protocol/handler payload rejection; source-to-runtime actor bridge and actor-syntax template remain pending | `cargo test -p garnet-check --test extended actor_protocol_rejects_nonsendable_payload_type`; `cargo test -p garnet-cli --test conformance_skeleton actor_sendable_rejects_nonsendable_protocol_payloads` |
