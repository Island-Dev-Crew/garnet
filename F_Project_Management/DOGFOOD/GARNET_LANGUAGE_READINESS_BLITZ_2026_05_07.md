...
| 8 | Phase 6 Memory Core + cache security | raises Memory Core from reference stores toward production-grade semantics without leaving cache/privacy gaps invisible | Phase 6D bounded root-buffer trial-deletion, finalization-order, and safe-mode exclusion fixtures active; Phase 6F cache episode path privacy gate active; Phase 6G same-cache foreign-key and copied-cache replay warning gates active; next: production ARC allocator integration and broader strategy replay/concurrency stress |
...
| Memory Core | Phase 6D bounded root-buffer trial-deletion, finalization-order, and safe-mode exclusion fixtures active; stores remain reference implementations; Phase 6F prevents absolute project/external paths from being persisted in compiler-as-agent cache episodes; Phase 6G warns while ignoring foreign machine-key and copied-cache replay episodes | production ARC allocator integration, persistence/privacy, and broader strategy replay/concurrency gates active |
...
| P2 | Production Memory Core ARC integration and broader strategy replay/concurrency stress tests | follow-up milestone branch |
