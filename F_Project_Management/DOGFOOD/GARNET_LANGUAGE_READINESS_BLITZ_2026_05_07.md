...
| 5 | Phase 3 actors + Sendable | makes agent-native examples executable instead of Rust-runtime-only | Phase 3C active: nonsendable actor payload rejection, managed actor handler dispatch, managed actor addresses, and bounded mailbox source calls; actor syntax template smoke still pending |
