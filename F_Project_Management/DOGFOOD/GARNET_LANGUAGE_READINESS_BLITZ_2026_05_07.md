...
| 8 | Phase 6 Memory Core + cache security | raises Memory Core from reference stores toward production-grade semantics without leaving cache/privacy gaps invisible | Phase 6D bounded root-buffer trial-deletion, finalization-order, and safe-mode exclusion fixtures active; Phase 6F cache episode path privacy gate active; Phase 6G same-cache foreign-key and copied-cache replay warning gates active; Phase 6H copied `strategies.db` quarantine and bounded concurrent episode append stress active; next: production ARC allocator integration and long-running cache soak |
...
| P2 | Production Memory Core ARC integration and long-running cache write soak tests | follow-up milestone branch |
