...
| `C_Language_Specification/GARNET_CAPABILITY_ENFORCEMENT_SCOPE.md` | capability enforcement scope fence (normative) | the exact line between the check-time CapCaps guarantee — which reaches named call chains only, misses a wholly unannotated recursive cycle even there, and is not run by `garnet run` at all (U-91) — and the bounded run-time / OS-boundary enforcement: declared/checker-only (`time::*`, `uuid` v4/v7), runtime-gated (12 `Guard::Gate`), entry-gated (3 `Guard::GateEntry`), declared-only-no-bridge (`ffi`, `net_internal`), unbridged (`net::tcp_listen`/`udp_bind`), OS-sandboxed (Linux-seccomp reference only, `enforced:false`), and caps-invisible (`memory::*`). Names what public copy may and may not say (no "universal `@caps` runtime enforcement", no "no ambient authority, ever"). Written for S114 acceptance condition #4. |
