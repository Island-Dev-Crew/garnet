...
| D7 | partial | Carry capabilities in a typed core IR and re-check after every lowering pass. | `F_Project_Management/W_REBUILD/W_REBUILD_SPEC.md` records the architecture criterion, but the current runtime does not provide per-pass typed-core re-check evidence. Reference semantics are not a production claim. |
