...
fn max_depth_in_range_passes() {
    ...
    assert!(!r.errors.iter().any(|e| matches!(e, CheckError::AnnotationError(_))));
}
...
fn safe_module_var_use_flagged() {
    let r = check("@safe\ndef bad() { var x = 1\n x }");
    assert!(r.errors.iter().any(|e| matches!(e, CheckError::SafeModeViolation(m) if m.contains("var"))));
}
...
fn safe_module_raise_flagged() {
    ...
    "#);
    assert!(r.errors.iter().any(|e| matches!(e, CheckError::SafeModeViolation(m) if m.contains("raise"))));
}
...
fn safe_module_try_flagged() {
    ...
    "#);
    assert!(r.errors.iter().any(|e| matches!(e, CheckError::SafeModeViolation(m) if m.contains("try"))));
}

#[test]
fn managed_module_var_allowed() {
    let r = check("def ok() { var x = 1\n x }");
    assert!(!r.errors.iter().any(|e| matches!(e, CheckError::SafeModeViolation(_))));
}
